...
const getVideoWM = async (url) => {
	...
	const obj = res.aweme_list[0];
	
    const urlMedia = obj.video.download_addr.url_list[0]
...
const getVideoNoWM = async (url) => {
    ...
    const urlMedia = obj.video.play_addr.url_list[0]
	const description = obj.desc;
...
const loadCookie = (page) => {
	fs.readFile('cookies.json', (error, cookieJson) => {
		if (error) {
			console.error(chalk.red("[X] Error loading cookies:", error));
			return;
		}
		...
		if (size > 0) {
			page.setCookie(...cookies)
				...
				.catch((error) => {
					console.error(chalk.red("[X] Failed to set cookies:", error));
				});
		} else {
			console.log(chalk.red("[X] Failed to load cookies!"));
		}
	});
};
...
const getListVideoByUsername = async (username) => {
	...
	const scraperDelay = 30000; // Adjust the delay if needed
	
	console.log(chalk.green("[*] You will be landed to main page."));
	console.log(chalk.green(`[*] You have ${Math.floor(scraperDelay / 1000)} seconds to load some content and solve captchas if any appear.`));
	console.log(chalk.green("[*] After that scraping will begin."));
	
    await delay(scraperDelay);
    await page.goto(baseUrl);
	
	console.log(chalk.green("[*] Scraping started"));
    console.log(chalk.green("[*] Getting videos from: " + username));
    ...
    while (loop) {
		listVideo = await page.evaluate(() => {
			...
			listVideo = parsedJSON.ItemList['user-post'].list
			return listVideo.map(function (i) { return window.location.href + '/video/' + i })
		})
		
        console.log(chalk.green(`[*] Video count: ${listVideo.length}`));
        ...
        await page.waitForFunction(
            ...
            { timeout: 6000 } // Increase the timeout if needed
        ).catch(() => {
            console.log(chalk.yellow("[!] No more videos found."));
...
(async () => {    
	...
	// It doesn't save the existing data
	fs.readFile('videoData.json', 'utf8', (err, data) => {
		if (err)
		{
			if (err.code === "ENOENT")
				console.log(chalk.yellow("[!] File videoData.json does not exist."));
			else
				console.error(chalk.red("[X] Error reading file videoData.json: ", err));
    ...
    downloadMediaFromList(listMedia) // videoData caching happens here
        .then(() => {
            console.log(chalk.green("[+] Downloading successful!"));
			console.log(chalk.green("[*] Video count: ", Object.keys(videoData.videos).length));

			fs.writeFile('videoData.json', JSON.stringify(videoData, null, 2), 'utf8', (err) =>
			{
				if (err)
				{
					console.error(chalk.red("[X] Error writing videoData.json: " + err));
					return;
				}
				else
					console.log(chalk.green("[+] Video data written successfully!"));
			});
        })
        .catch(err => {
